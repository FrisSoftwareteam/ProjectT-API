...
$jsonPayloads = [
    ...
    'ShareholderController@storeWithRegisterAndShares' => ['shareholder' => ['holder_type' => 'individual', 'first_name' => 'Ada', 'last_name' => 'Okafor', 'email' => 'ada.register@example.com', 'phone' => '08012345003', 'status' => 'active'], 'addresses' => [['address_line1' => '1 Marina Road', 'city' => 'Lagos', 'state' => 'Lagos', 'country' => 'Nigeria', 'is_primary' => true]], 'register_account' => ['register_id' => '{{register_id}}', 'shareholder_no' => 'SH-000002', 'chn' => 'C123456790', 'cscs_account_no' => 'CSCS-000002', 'residency_status' => 'resident', 'kyc_level' => 'standard', 'status' => 'active'], 'share_class_id' => '{{share_class_id}}', 'initial_shares' => ['quantity' => '1000', 'source_type' => 'allotment', 'lot_ref' => 'LOT-002', 'acquired_at' => '2026-09-11', 'holding_mode' => 'demat']],
...
$queryExamples = [
    ...
    'CscsUploadController@exceptions' => ['resolution_status' => 'UNRESOLVED', 'search' => 'C111111111', 'per_page' => '50'],
    'CscsUploadController@transactions' => ['balance_status' => 'BALANCED', 'is_flagged' => 'false', 'page' => '1', 'per_page' => '50'],
...
function descriptionFor(array $route, string $action): string
{
    ...
    $description .= match ($action) {
        ...
        'ShareholderController@index' => "\n\n`search` matches shareholder names/contact fields and also shareholder numbers, CHNs, and CSCS account numbers. Use `register_id` when resolving a CSCS exception.",
        'CscsUploadController@transactions' => "\n\nEach transaction includes UI-ready `debit_account`, `credit_account`, `quantity`, `risk`, and `resolution` objects in addition to totals, flags, and raw legs.",
        'CscsUploadController@transaction' => "\n\nReturns the same UI-ready transaction contract as the list: account display objects, quantity, risk, resolution, totals, flags, and raw legs.",
        'CscsUploadController@exceptions' => "\n\nEach exception includes severity, blocking state, parsed record values, allowed and suggested resolutions, matched account, and resolution history. `meta.exception_counts` contains total, blocking, warning, resolved, and remaining counts.",
        'CscsUploadController@preview' => "\n\nConsolidated checker payload containing named account effects, proposed new accounts, security mappings, review summary, approval timeline, and comments.",
        ...
        'CscsUploadController@verificationSummary' => "\n\nUse this consolidated response for the post-posting verification screen. `data.comparison` contains approved, actual, variance, status, and matched values. Display success only when `data.verification_status` is `VERIFIED` and `data.all_checks_passed` is true.",
